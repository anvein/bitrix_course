(function ()
{
	...
	var s = BX.Landing.Utils.makeFilterablePopupMenu;
	var o = BX.Landing.Utils.makeSelectablePopupMenu;
	var a = BX.Landing.Utils.style;
	var d = BX.Landing.Utils.encodeDataValue;
	var l = BX.Landing.UI.Tool.Menu;
	BX.Landing.UI.Panel.Top = function (t, n)
	{
		...
		e(this.desktopButton, "click", this.onDesktopSizeChange);
		e(this.tabletButton, "click", this.onTabletSizeChange);
		e(this.mobileButton, "click", this.onMobileSizeChange);
		e(this.iframe.contentDocument, "click", this.onIframeClick);
		e(this.undoButton, "click", this.onUndo);
		e(this.redoButton, "click", this.onRedo);
		e(top.document, "keydown", this.onKeyDown);
		i(top.document, "iframe:keydown", this.onKeyDown);
		i(top.window, "BX.Landing.History:init", this.adjustHistoryButtonsState);
		i(top.window, "BX.Landing.History:update", this.adjustHistoryButtonsState);
		var s = parseInt(BX.Landing.Main.getInstance().options.sites_count);
		var o = parseInt(BX.Landing.Main.getInstance().options.pages_count);
		if (s > 1)
		{
			e(this.siteButton, "click", this.onSiteButtonClick)
		}
		if (o > 1)
		{
			e(this.pageButton, "click", this.onPageButtonClick)
		}
	...
	BX.Landing.UI.Panel.Top.prototype = {
		...
		onKeyDown: function (t)
		{
			...
			if (n === 90 && (top.window.navigator.userAgent.match(/win/i) ? t.ctrlKey : t.metaKey))
			{
				if (t.shiftKey)
				{
					t.preventDefault();
		...
		onUndo: function ()
		{
			if (BX.Landing.History.getInstance().canUndo())
			{
				...
				n(this.undoButton, "landing-ui-onload");
				BX.Landing.History.getInstance().undo().then(function ()
				{
					this.getLoader().hide();
					t(this.undoButton, "landing-ui-onload")
				}.bind(this))
			} else
			{
				this.getLoader().hide();
		...
		onRedo: function ()
		{
			if (BX.Landing.History.getInstance().canRedo())
			{
				...
				n(this.redoButton, "landing-ui-onload");
				BX.Landing.History.getInstance().redo().then(function ()
				{
					this.getLoader().hide();
					t(this.redoButton, "landing-ui-onload")
				}.bind(this))
			} else
			{
				this.getLoader().hide();
		...
		getLoader: function ()
		{
			if (this.loader === null)
			{
				...
				a(this.loader.layout.querySelector(".main-ui-loader-svg-circle"), {"stroke-width": "4px"})
			}
			return this.loader
		},
		adjustHistoryButtonsState: function (t)
		{
			if (t.canUndo())
			{
		...
		onSiteButtonClick: function (t)
		{
			t.preventDefault();
			if (!this.siteMenu)
			{
				var n = new BX.Loader({size: 40});
				this.siteMenu = new l({
					id: "site_list_menu",
				...
				n.show(this.siteMenu.popupWindow.contentContainer);
				var i = {
					siteId: BX.Landing.Main.getInstance().options.site_id,
					landingId: BX.Landing.Main.getInstance().id,
					filter: {"=TYPE": BX.Landing.Main.getInstance().options.params.type}
				};
				BX.Landing.UI.Panel.URLList.getInstance().getSites(i).then(function (t)
				{
					s(this.siteMenu);
					o(this.siteMenu);
					t.forEach(function (t)
					{
						this.siteMenu.addMenuItem({
							id: t.ID, text: d(t.TITLE), items: function ()
							{
								var n = [];
								var i = BX.Landing.Main.getInstance().options.params.sef_url.site_edit;
								var e = BX.Landing.Main.getInstance().options.params.sef_url.site_show;
								n.push({
		...
		onPageButtonClick: function (t)
		{
			...
			if (!this.pageMenu)
			{
				...
				n.show(this.pageMenu.popupWindow.contentContainer);
				var i = {
					siteId: BX.Landing.Main.getInstance().options.site_id,
					landingId: BX.Landing.Main.getInstance().id,
					filter: {"=TYPE": BX.Landing.Main.getInstance().options.params.type}
				};
				BX.Landing.UI.Panel.URLList.getInstance().getLandings(i.siteId, i).then(function (t)
				{
					s(this.pageMenu);
					o(this.pageMenu);
					t.forEach(function (t)
					{
						if (!t.FOLDER_ID)
						{
							this.pageMenu.addMenuItem({
								...
								{
									var n = [];
									var i = BX.Landing.Main.getInstance().options.params.sef_url.landing_edit;
									var e = BX.Landing.Main.getInstance().options.params.sef_url.landing_view;
									if (t.FOLDER === "Y")
									{
										var s = BX.Landing.Main.getInstance().options.params.sef_url.site_show;
										n.push({
											text: BX.message("LANDING_ENTITIES_MENU_PAGES_LIST"),
									...
									n.push({
										...
										href: e.replace("#site_show#", t.SITE_ID).replace("#landing_edit#", t.ID)
									});
									n.push({
										text: BX.message("LANDING_ENTITIES_MENU_PAGES_SETTINGS"),
										href: i.replace("#site_show#", t.SITE_ID).replace("#landing_edit#", t.ID)
									});
									return n
								}()
							})
