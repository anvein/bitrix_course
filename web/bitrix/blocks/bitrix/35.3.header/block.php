<header class="landing-block landing-block-menu g-bg-white g-pt-20 g-pb-20 g-brd-bottom g-brd-gray-light-v4">
	<div class="text-center text-lg-left">
		<div class="row flex-lg-row align-items-center justify-content-lg-start">
			<div class="col-6 col-sm-3 col-lg-2">
				<!-- Logo -->
				<a href="#system_mainpage" class="navbar-brand landing-block-node-menu-logo-link">
					<img class="landing-block-node-logo img-fluid g-max-width-180"
						 src="/bitrix/images/landing/logos/real-estate-logo.png" alt="Logo">
				</a>
				<!-- End Logo -->
			</div>

			<div class="col-6 col-sm-9 col-lg-10">
				<div class="row align-items-center">
					<div class="landing-block-node-card col-sm g-brd-right--sm g-brd-gray-light-v4">
						<div class="g-pa-10--lg">
								<span class="landing-block-node-card-icon icon icon-screen-smartphone g-valign-middle g-font-size-18
									g-color-primary g-mr-5"></span>
							<div class="landing-block-node-card-title d-inline-block text-uppercase g-font-size-13">
								Call Us
							</div>
							<div class="landing-block-node-card-text d-block g-pl-25">
								<a href="tel:+469548521" class="landing-block-node-card-link g-color-gray-dark-v5 d-block g-font-weight-700">+469 548 521</a>
							</div>
						</div>
					</div>

					<div class="landing-block-node-card col-sm g-hidden-md-down g-brd-right--sm g-brd-gray-light-v4">
						<div class="g-pa-10--lg">
								<span class="landing-block-node-card-icon icon icon-clock g-valign-middle g-font-size-18
									g-color-primary g-mr-5"></span>
							<div class="landing-block-node-card-title d-inline-block text-uppercase g-font-size-13">
								Opening time
							</div>
							<div class="landing-block-node-card-text d-block g-pl-25">
								<span style="font-weight: bold;">Mon-Sat: 08.00 - 18.00</span>
							</div>
						</div>
					</div>

					<div class="landing-block-node-card col-sm g-hidden-sm-down g-brd-right--sm g-brd-gray-light-v4">
						<div class="g-pa-10--lg">
								<span class="landing-block-node-card-icon icon icon-envelope g-valign-middle g-font-size-18
									g-color-primary g-mr-5"></span>
							<div class="landing-block-node-card-title d-inline-block text-uppercase g-font-size-13">
								Email us
							</div>
							<div class="landing-block-node-card-text d-block g-pl-25">
								<a href="mailto:market@info.com" class="landing-block-node-card-link d-block g-color-gray-dark-v5 g-font-weight-700">market@info.com</a>
							</div>
						</div>
					</div>

				</div>
			</div>
		</div>
	</div>
</header>